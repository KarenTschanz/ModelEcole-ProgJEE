...
class LivreDAOTest {
	...
	void testGetLivreByTitre() {
		ArrayList<Livre> lstref = new ArrayList<Livre> ();
		ArrayList<Livre> lstresult = new ArrayList<Livre> ();
		
		Livre e_ref1 = new Livre("ISBN10001","", "Seul au monde", null);

		lstref.add(e_ref1);
		
		try {
			lstresult = LivreDAO.getLivreByTitre("Seul au monde");
